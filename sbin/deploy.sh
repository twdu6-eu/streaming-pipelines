...
nohup spark-submit --master yarn --deploy-mode cluster --queue streaming --class com.tw.apps.StationLocationApp --name StationStatusSaverApp --packages org.apache.spark:spark-sql-kafka-0-10_2.11:2.3.0 --driver-memory 500M --conf spark.streaming.stopGracefullyOnShutdown=true --conf spark.executor.memory=2g --conf spark.cores.max=1 /tmp/tw-raw-data-saver_2.11-0.0.1.jar ${ZOOKEEPER_CONFIG} "/tw/stationStatus" 1>/tmp/raw-station-status-data-saver.log 2>/tmp/raw-station-status-data-saver.error.log &

nohup spark-submit --master yarn --deploy-mode cluster --queue streaming --class com.tw.apps.StationLocationApp --name StationInformationSaverApp --packages org.apache.spark:spark-sql-kafka-0-10_2.11:2.3.0 --driver-memory 500M --conf spark.streaming.stopGracefullyOnShutdown=true --conf spark.executor.memory=2g --conf spark.cores.max=1 /tmp/tw-raw-data-saver_2.11-0.0.1.jar ${ZOOKEEPER_CONFIG} "/tw/stationInformation" 1>/tmp/raw-station-information-data-saver.log 2>/tmp/raw-station-information-data-saver.error.log &

nohup spark-submit --master yarn --deploy-mode cluster --queue streaming --class com.tw.apps.StationLocationApp --name StationDataSFSaverApp --packages org.apache.spark:spark-sql-kafka-0-10_2.11:2.3.0 --driver-memory 500M --conf spark.streaming.stopGracefullyOnShutdown=true --conf spark.executor.memory=2g --conf spark.cores.max=1 /tmp/tw-raw-data-saver_2.11-0.0.1.jar ${ZOOKEEPER_CONFIG} "/tw/stationDataSF" 1>/tmp/raw-station-data-sf-saver.log 2>/tmp/raw-station-data-sf-saver.error.log &
...
nohup spark-submit --master yarn --deploy-mode cluster --queue streaming --class com.tw.apps.StationApp --name StationApp --packages org.apache.spark:spark-sql-kafka-0-10_2.11:2.3.0  --driver-memory 500M --conf spark.executor.memory=2g --conf spark.cores.max=1 --conf spark.streaming.stopGracefullyOnShutdown=true /tmp/tw-station-consumer_2.11-0.0.1.jar ${ZOOKEEPER_CONFIG} 1>/tmp/station-consumer.log 2>/tmp/station-consumer.error.log &

nohup spark-submit --master yarn --deploy-mode cluster --queue streaming --class com.tw.apps.StationApp --name StationTransformerNYC --packages org.apache.spark:spark-sql-kafka-0-10_2.11:2.3.0  --driver-memory 500M --conf spark.executor.memory=2g --conf spark.cores.max=1 --conf spark.streaming.stopGracefullyOnShutdown=true /tmp/tw-station-transformer-nyc_2.11-0.0.1.jar ${ZOOKEEPER_CONFIG} 1>/tmp/station-transformer-nyc.log 2>/tmp/station-transformer-nyc.error.log &
